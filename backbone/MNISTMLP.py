...
class MNISTMLP(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        self.eq_head.apply(xavier)
        self.inv_head.apply(xavier)
    ...
    def forward(self, x: torch.Tensor, return_features = False) :
        ...
        #return self.classifier(out), out, out, F.normalize(out, dim=1)
        if return_features:
            return self.classifier(out), self.eq_head(out)#, F.normalize(self.inv_head(out), dim=1)
            #return self.classifier(out), self.eq_head(out), F.normalize(self.inv_head(out), dim=1), F.normalize(out, dim=1)
        else:
